...
BUFFER_SIZE = 30_000
MIN_REPLAY_SIZE = 40 # 5000 #50_000
NUM_ENVS = 1
LOGGING_INTERVAL = 12 # 150
TAU = 2e-3 # SOFT NET UPDATE
GAMMA = 0.99
IMG_HW = [72, 128]
ACTION_SIZE = 3
CLIP_ACTION = [-1.5, 1.5]
...
SAVE_WEIGHTS = './saved_weights/ac/'
LOGG_TB_DIR = "./logs/ac_maze/"
...
class Actor(nn.Module):
    ...
    def forward(self, state, velocity=None):
        ...
        actions_pred = self.actor(out_shared)
    ...
    def get_shared_net(self):
        ...
        if not self.gru_shaped:
            self.gru_input_size = self.shared_net.to(device)(torch.rand(2, 2, self.input_ch,  self.img_hw[0],  self.img_hw[1], device=device)).size()[-1]
...
class DDPG_Agent(nn.Module):
    ...
    def train(self):
        ...
        for i, sample in enumerate(batch):
            ...
            dones[i] = torch.as_tensor(sample[4], device=device, dtype=torch.bool)
        ...
        critic_value = self.critic.forward( state = images,velocity=vels, action = actions)
...
def training_ddpg(env, logg_tb, save_path, seq_size):

    logger.info(f'---------- device  =  {device} ')
    tb_summary = SummaryWriter(logg_tb)
    rddpg_agent = DDPG_Agent(save_path=save_path, tau=TAU, input_ch=3, gamma=GAMMA, memory_size=BUFFER_SIZE, \
                             batch_size = BATCH_SIZE, img_hw = IMG_HW, seq_size = seq_size, action_size=ACTION_SIZE,\
                             min_max_actions = CLIP_ACTION , velocity_size = VELOCITY_SIZE).to(device)
    ...
    for i in range(MIN_REPLAY_SIZE):
        ...
        try:
            ...
            rddpg_agent.replay_buffer.append((state, action, reward, new_state, done))
            state = new_state
    ...
    # main training:
    global_step = 0
    actor_sum_loss_episodes, critic_sum_loss_episodes, sum_reward_episodes = 0, 0, 0
    time_limit = 600
    for episode in itertools.count():
        actor_sum_loss_per_ep, critic_sum_loss_per_ep , sum_reward_per_episode = 0, 0, 0
        ...
        try:
            while not done and step_epoch < time_limit :
                ...
                observe, reward, terminated, truncated, info = env.step(action[0])  # step from noisy action / 1d list
                logger.info(f'actions = {action[0]}, reward = {reward}')
        ...
        if episode % LOGGING_INTERVAL == 0:
            ...
            # if better - do checkpoint for best
            ...
            # save stats for actor / critic loss


def train_DDPG(logg_tb, save_path, seq_size,  height_airsim_restart_positions):
    ...
    res = 0
    try: # '../../../../../Documents'
        res = training_ddpg(env, logg_tb=logg_tb, save_path=save_path, seq_size=seq_size)
...
def main_ddpg(EPOCHS, env_name):
    for epoch in range(EPOCHS):

        LOGG_TB_DIR = f"logs/dueling_ddqn_{env_name}/restart_exe_{epoch}/"  # <------- change here
        SAVE_PATH = f"./saved_weights/dueling_ddqn_{env_name}/restart_{epoch}/"  # <------- change here
        ...
        if len(rewards_logs) > 1:
            ...
            load_path = f"./saved_weights/dueling_ddqn_{env_name}/{restart_n}"  # <------- change here
            logger.info(f'Found best log {load_path}')

        train_DDPG(logg_tb=LOGG_TB_DIR, save_path=SAVE_PATH, epoch=epoch, load_path=load_path,
                            reward_loggs=rewards_logs, env_name=env_name, height_airsim_restart_positions= [-0.8339])
        time.sleep(5)

def convert2onnx(model, input):
    onnx_program = torch.onnx.export(model, input, "actor.onnx")
    # onnx_program.save("actor.onnx")

def check_ddpg():
    img_hw = [72, 128]
    state = torch.rand(6, 5, 3, img_hw[0], img_hw[1],device=device)  # batch_size, time_steps, input_size
    action = torch.rand(6,5, 5, device=device)
    velocity = torch.rand(6, 5, 4, device=device)
    seq_size = 5

    model = DDPG_Agent(save_path = SAVE_WEIGHTS, tau=TAU, input_ch=3, gamma=GAMMA, memory_size=BUFFER_SIZE, \
                             batch_size = BATCH_SIZE, img_hw = IMG_HW, seq_size = seq_size, action_size=ACTION_SIZE,\
                             min_max_actions = CLIP_ACTION , velocity_size = VELOCITY_SIZE).to(device)
...
def check_td():
    ...
    # x = torch.rand(4, 20, 3, 64, 64, device=device) # batch_size, time_steps, C, H, W
    x = torch.rand(4, 20, 16, device=device) # batch_size, time_steps, input_size (after flatten)
    # model = TimeDistributed(nn.Conv2d, time_steps=20, in_channels=3, out_channels=8, kernel_size=(3, 3), stride=2, padding=1, bias = True).to(device) # in_channels, out_channels, kernel_size, stride, padding, bias=True
    model = TimeDistributed(nn.GRU, time_steps=20 , input_size= 16 , hidden_size = 64, bias=False, batch_first=True).to(device)
    output = model(x)


    print(output.size())  # torch.Size([4, 20, 8, 32, 32])

    print("number of parameters : ", sum([p.numel() for p in model.parameters()]))


if __name__=="__main__":
    train_DDPG(logg_tb = LOGG_TB_DIR, save_path = SAVE_WEIGHTS, seq_size = 5,  height_airsim_restart_positions = [-0.8339])
